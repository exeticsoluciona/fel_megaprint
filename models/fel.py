...
class FelMegaprint(models.Model):
    ...
    def dteAnulaDocumento(self):
        ...
        fecha_hora_hoy = fields.Datetime.context_timestamp(factura, timestamp=datetime.now()).strftime('%Y-%m-%dT%H:%M:%S-06:00')
        ...
        DatosGenerales = etree.SubElement(AnulacionDTE, DTE_NS+"DatosGenerales", ID="DatosAnulacion", NumeroDocumentoAAnular=factura.firma_fel, NITEmisor=factura.journal_id.direccion_fel.vat.replace("-",""), IDReceptor=nit_receptor, FechaEmisionDocumentoAnular=fecha_hora, FechaHoraAnulacion=fecha_hora_hoy, MotivoAnulacion=motivo_anulacion)
    ...
    def dteDocumento(self):
        ...
        Emisor = etree.SubElement(DatosEmision, DTE_NS+"Emisor", AfiliacionIVA="GEN", CodigoEstablecimiento=factura.journal_id.direccion_fel.codigo_establecimiento, CorreoEmisor=factura.journal_id.direccion_fel.email or '', NITEmisor=factura.journal_id.direccion_fel.vat.replace('-',''), NombreComercial=factura.journal_id.direccion_fel.name, NombreEmisor=factura.journal_id.direccion_fel.name)
        ...
        if tipo_documento_fel in ['NDEB', 'NCRE'] or tipo_documento_fel in ['FCAM'] or (tipo_documento_fel in ['FACT', 'FCAM'] and factura.tipo_gasto == 'importacion') or tipo_documento_fel in ['FESP']:
            ...
            if tipo_documento_fel in ['NDEB', 'NCRE']:
                ...
                if factura.factura_org_id.numero_fel:
                    ReferenciasNota = etree.SubElement(Complemento, CNO_NS+"ReferenciasNota", FechaEmisionDocumentoOrigen=str(factura.factura_org_id.invoice_date), MotivoAjuste="-", NumeroAutorizacionDocumentoOrigen=factura.factura_org_id.firma_fel, NumeroDocumentoOrigen=factura.factura_org_id.numero_fel, SerieDocumentoOrigen=factura.factura_org_id.serie_fel, Version="0.0", nsmap=NSMAP_REF)
                else:
                    ReferenciasNota = etree.SubElement(Complemento, CNO_NS+"ReferenciasNota", RegimenAntiguo="Antiguo", FechaEmisionDocumentoOrigen=str(factura.factura_org_id.invoice_date), MotivoAjuste="-", NumeroAutorizacionDocumentoOrigen=factura.factura_org_id.firma_fel, NumeroDocumentoOrigen=factura.factura_org_id.name.split("-")[1], SerieDocumentoOrigen=factura.factura_org_id.name.split("-")[0], Version="0.0", nsmap=NSMAP_REF)
...
class AccountJournal(models.Model):
    ...
    tipo_documento_fel = fields.Selection([('FACT', 'Factura'), ('FCAM', 'Factura Cambiaria'), ('FESP', 'Factura Especial'), ('NABN', 'Nota de Abono'), ('NDEB', 'Nota de Debito'), ('NCRE', 'Nota de Credito')], 'Tipo de Documento FEL', copy=False)
...
class ResCompany(models.Model):
    ...
    url_api_fel = fields.Char('URL del API Crea DTE', help="URL del API que se consume para generar los documentos FEL.")
    url_api_token_fel = fields.Char('URL del API Solicita Token DTE', help="URL del API que se consume para solictar token.")
    url_api_firma_fel = fields.Char('URL del API Firma DTE', help="URL del API que se consume para generar firmar documento FEL.")
    url_api_anula_fel = fields.Char('URL del API anula DTE', help="URL del API que se consume para anular los documentos FEL.")
    url_api_pdf_fel = fields.Char('URL del API Genera PDF', help="URL del API que se consume para generar archivo PDF.")
    dev_url_api_fel = fields.Char('URL del API dev_ Crea DTE', help="URL del API que se consume para generar los documentos FEL.")
    dev_url_api_token_fel = fields.Char('URL del API dev_ Solicita Token DTE', help="URL del API que se consume para solictar token.")
    dev_url_api_firma_fel = fields.Char('URL del API dev_ Firma DTE', help="URL del API que se consume para generar firmar documento FEL.")
    dev_url_api_anula_fel = fields.Char('URL del API dev_ anula DTE', help="URL del API que se consume para anular los documentos FEL.")    
    dev_url_api_pdf_fel = fields.Char('URL del API dev_ Genera PDF', help="URL del API que se consume para generar archivo PDF.")    
